-- Fix: ensure an active PlanckPass season + harden XP award for client/SQL callers
-- (includes ambiguous out-param / column fix for season_id, xp_total, amount)

-- Activate the newest season if none is active (common after create without activate flag)
do $$
declare
  v_id uuid;
begin
  if not exists (select 1 from public.planckpass_seasons where is_active = true) then
    select id into v_id
    from public.planckpass_seasons
    order by created_at desc nulls last
    limit 1;

    if v_id is not null then
      update public.planckpass_seasons
...
-- Harden award: security definer + bypass RLS; avoid OUT-param / column ambiguity
