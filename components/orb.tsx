...
export default function Orb({
  ...
  useEffect(() => {
    ...
    if (!gl) {
      ...
      container.style.background = 'radial-gradient(circle at center, rgba(168, 85, 247, 0.1) 0%, rgba(59, 130, 246, 0.1) 50%, transparent 100%)';
      return;
    }
    ...
    const program = new Program(glContext, {
      ...
      uniforms: {
        iTime: { value: 0 },
        iResolution: {
          value: new Vec3(glContext.canvas.width, glContext.canvas.height, glContext.canvas.width / glContext.canvas.height)
        },
        hue: { value: hue },
        rot: { value: 0 }
      }
    ...
    function resize() {
      if (!container) return;
      const dpr = Math.min(window.devicePixelRatio || 1, 2); // Limit DPR for mobile performance
