...
import { MathProblemCard } from "./problem-card"
import { Skeleton } from "@/components/ui/skeleton"

interface MathProblemGridProps {
...
export function MathProblemGrid({ problems, loading, onSelectProblem }: MathProblemGridProps) {
  if (loading) {
    return (
  ...
  if (problems.length === 0) {
    return (
      <div className="rounded-3xl border border-white/10 bg-white/[0.03] px-8 py-12 text-center text-white/70 shadow-[0px_24px_70px_-40px_rgba(0,0,0,1)]">
        <p className="text-lg text-white">Nu am găsit probleme care să se potrivească filtrelor.</p>
        <p className="mt-3 text-sm text-white/60">Ajustează filtrele sau resetează-le.</p>
      </div>
    )
  }
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {problems.map((problem) => (
        <MathProblemCard key={problem.id} problem={problem} onSelect={onSelectProblem} />
      ))}
    </div>
  )
}
