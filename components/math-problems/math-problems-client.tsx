...
import { MathProblemFilterBar } from "./filter-bar"
import { MathProblemGrid } from "./problem-grid"

const DEFAULT_FILTERS: MathProblemFiltersState = {
  search: "",
  class: "Toate",
  difficulty: "Toate",
}

interface MathProblemsClientProps {
  initialProblems: MathProblem[]
  initialMeta: MathProblemsApiResponse["meta"]
  initialFacets: MathProblemFacets | null
  pageSize: number
}

export function MathProblemsClient({
  initialProblems,
  initialMeta,
  initialFacets,
  pageSize,
}: MathProblemsClientProps) {
  const [filters, setFilters] = useState<MathProblemFiltersState>(DEFAULT_FILTERS)
  const [currentPage, setCurrentPage] = useState<number>(1)
  const [problems, setProblems] = useState<MathProblem[]>(initialProblems)
  const [meta, setMeta] = useState<MathProblemsApiResponse["meta"]>(initialMeta)
  const [facets, setFacets] = useState<MathProblemFacets | null>(initialFacets)
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      try {
        const response = await fetch(`/api/math-problems?${params.toString()}`, {
        ...
        const payload = (await response.json()) as MathProblemsApiResponse
        ...
        console.error("[math-problems-client] Failed to fetch:", err)
        setError(err instanceof Error ? err.message : "A apărut o eroare la încărcarea problemelor.")
      } finally {
        setLoading(false)
      }
    }, effectiveDelay)
  ...
  const handleFiltersChange = (nextFilters: MathProblemFiltersState) => {
  ...
  return (
    <div className="space-y-10">
      ...
      <MathProblemGrid problems={problems} loading={loading} />
