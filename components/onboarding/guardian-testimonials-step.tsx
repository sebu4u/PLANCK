...
import {
  getGuardianTestimonials,
  getGuardianTestimonialsCopy,
  type GuardianTestimonial,
} from "@/lib/guardian-testimonials"
import type { GuardianRole } from "@/lib/guardian-onboarding"

type GuardianTestimonialsStepProps = {
  role: GuardianRole
}

function StarRating({ rating }: { rating: GuardianTestimonial["rating"] }) {
...
function TestimonialCard({
  ...
  testimonial: GuardianTestimonial
...
export function GuardianTestimonialsStep({ role }: GuardianTestimonialsStepProps) {
  const testimonials = getGuardianTestimonials(role)
  const copy = getGuardianTestimonialsCopy(role)

  return (
    <div className="mx-auto flex w-full max-w-[960px] translate-y-[5vh] flex-col gap-4 lg:translate-y-0 lg:flex-row lg:items-start lg:gap-10 xl:gap-14">
