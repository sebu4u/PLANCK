...
import { getLearningPathItemLabel } from "@/lib/subject-map/supabase-learning-map"
import type {
  SubjectMapChapter,
  SubjectMapLesson,
...
export interface SubjectMapManagerProps {
  apiPath: string
  mapPublicPath: string
  lessonIdField: string
  learningPathMarker: string | null
}

export function SubjectMapManager({
  apiPath,
  mapPublicPath,
  lessonIdField,
  learningPathMarker,
}: SubjectMapManagerProps) {
  const [routes, setRoutes] = useState<SubjectMapRoute[]>([])
  const [chapters, setChapters] = useState<SubjectMapChapter[]>([])
  const [lessons, setLessons] = useState<SubjectMapLesson[]>([])
  const [assignments, setAssignments] = useState<SubjectMapLessonItemAssignment[]>([])
  ...
  const [editLessonType, setEditLessonType] = useState<SubjectMapLesson["lesson_type"]>("invata")
  ...
  const refreshData = useCallback(
    async (options?: { showFullScreenLoader?: boolean }) => {
      ...
      try {
        ...
        const [mapRes, lpRes] = await Promise.all([
          fetch(apiPath, {
        ...
        const nextRoutes: SubjectMapRoute[] = mapData.routes ?? []
        const nextChapters: SubjectMapChapter[] = mapData.chapters ?? []
        const nextLessons: SubjectMapLesson[] = mapData.lessons ?? []

        setRoutes(nextRoutes)
        setChapters(nextChapters)
        setLessons(nextLessons)
        setAssignments(mapData.assignments ?? [])
  ...
  const lessonAssignments = useMemo(
    () =>
      assignments
        .filter((assignment) => {
          const raw = assignment as SubjectMapLessonItemAssignment & Record<string, unknown>
          return raw.mapLessonId === selectedLessonId || raw[lessonIdField] === selectedLessonId
        })
        .sort((a, b) => a.order_index - b.order_index),
    [assignments, lessonIdField, selectedLessonId],
  )
  ...
  const learningPathTree = useMemo(() => {
    const filteredChapters = learningPathMarker
      ? lpChapters.filter((chapter) => chapter.problem_category === learningPathMarker)
      : lpChapters
  ...
  }, [lpChapters, lpLessons, lpItems, learningPathMarker])
  ...
  const apiRequest = useCallback(
    ...
    [apiPath, getAccessToken, refreshData],
  )
  ...
  const handleAssignItem = async (learningPathItemId: string) => {
    if (!selectedLessonId) {
      setError("Selectează mai întâi o lecție din secțiunea „Lecții” (hartă).")
      return
    }
    const ok = await apiRequest("POST", {
      type: "assignment",
      [lessonIdField]: selectedLessonId,
      map_lesson_id: selectedLessonId,
            ...
            {selectedLesson ? (
              <div className="mt-6 rounded-lg border border-white/10 bg-black/20 p-4">
                ...
                <p className="mb-4 text-xs text-gray-500">
                  Modificările apar pe harta de la {mapPublicPath}.
                </p>
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setEditLessonType(value as SubjectMapLesson["lesson_type"])}
                    >
                      <SelectTrigger className="bg-black/40 text-white">
                        <SelectValue />
                      </SelectTrigger>
