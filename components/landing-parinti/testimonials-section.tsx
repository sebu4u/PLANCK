...
function pickLandingTestimonials(): Array<HomepageTestimonial & { avatarSrc: string }> {
  const parents = HOMEPAGE_TESTIMONIALS.filter((t) => t.category === "parent")
  const students = HOMEPAGE_TESTIMONIALS.filter((t) => t.category === "student")
  const picked = [...parents, ...students].slice(0, 6)
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <FadeInUp className="mb-12 text-center">
          <h2 className="text-3xl font-black tracking-tight text-gray-900 sm:text-4xl">
            Rezultate la notă — nu marketing gol
          </h2>
          <p className="mt-3 text-base text-gray-500 sm:text-lg">
            Părinți care văd progresul acasă și elevi care și-au salvat media. Nume reale.
          </p>
        </FadeInUp>

        <div
          className="sm:hidden motion-reduce:hidden -mx-4 w-[calc(100%+2rem)] overflow-hidden select-none [touch-action:pan-y]"
          aria-label="Testimoniale părinți și elevi Planck"
        >
