...
import { BookOpen, Check, ChevronDown, Clock, Dumbbell, List, Loader2, Map, PenLine, Play, RotateCcw } from "lucide-react"
...
import { INFO_MAP_COMPLETED_LESSON_PARAM } from "@/lib/info-map-item-navigation"
import {
  INFO_CALENDAR_ENABLED,
  INFO_HUB_CARDS_ENABLED,
  INFO_INSIGHT_STARTER_CHIPS,
  INFO_LESSON_TYPE_LABEL,
  type InfoLessonType,
} from "@/lib/invata-info-config"
...
import {
  getInfoMapHref,
  type InfoMapPageData,
} from "@/lib/supabase-info-learning-map"
...
function buildInfoMapInsightContext(mapData: InfoMapPageData): string {
  const { selectedRoute, selectedChapter, lessons } = mapData
  const parts = [
    "Context: utilizatorul este pe harta lecțiilor de fizică (Planck Academy — /invata/info).",
    "Rolul tău: tutor de informatică pentru liceu; răspunde clar, pas cu pas, în română.",
  ]
  ...
  if (lessons.length > 0) {
    ...
    for (const lesson of lessons) {
      parts.push(
        `- ${lesson.title} (${INFO_LESSON_TYPE_LABEL[lesson.type]}, ${lesson.durationMinutes} min, status: ${lesson.status})`,
      )
    }
  }
...
function LessonTypeIcon({ type, className }: { type: InfoLessonType; className?: string }) {
  if (type === "exerseaza") {
...
function SidebarNav({
  ...
  mapData: InfoMapPageData
  onNavigate: (href: string) => void
}) {
  const { routes, chapters, selectedRoute, selectedChapter } = mapData

  return (
    <nav className="space-y-4" aria-label="Navigare informatică">
      ...
      <div>
        ...
        <div className="space-y-1">
          {routes.map((route) => {
            ...
            const href = `/invata/info?traseu=${encodeURIComponent(route.slug)}`
            ...
            <div className="space-y-1">
              {chapters.map((chapter) => {
                ...
                return (
                  <Link
                    key={chapter.id}
                    href={getInfoMapHref(selectedRoute.slug, chapter.slug)}
                    onClick={(event) => {
                      event.preventDefault()
                      onNavigate(getInfoMapHref(selectedRoute.slug, chapter.slug))
                    }}
                    className={cn(
        ...
        <div className="relative">
          <select
            ...
            onChange={(event) => {
              ...
              if (!nextRoute) return
              onNavigate(`/invata/info?traseu=${encodeURIComponent(nextRoute.slug)}`)
            }}
          >
            {routes.map((route) => (
        ...
        <label className="block">
          ...
          <div className="relative">
            <select
              ...
              onChange={(event) => {
                onNavigate(getInfoMapHref(selectedRoute.slug, event.target.value))
              }}
            >
              {chapters.map((chapter) => (
...
function reconcileInfoMapLessonStatuses(
  lessons: MapLesson[],
...
function isInfoLessonSelectable(lesson: MapLesson): boolean {
    ...
    >
      ...
      <div className={cn("flex flex-wrap items-center gap-2", isUpcoming ? "mt-1.5" : "mt-2.5")}>
        {isCompleted ? (
          ...
          >
            {INFO_LESSON_TYPE_LABEL[lesson.type]}
          </span>
        )}
        <span
          className={cn(
...
function DesktopMap({
  ...
  revealKey: number
  onEnterLesson: (href: string) => void
  onOpenItemsList: (lesson: MapLesson) => void
  isEnteringLesson: boolean
}) {
    ...
    >
      ...
      {lessons.map((lesson, index) => (
        <DesktopLessonMarker
          ...
          onOpenItemsList={() => onOpenItemsList(lesson)}
...
function DesktopLessonMarker({
  ...
  const isSelectable = isInfoLessonSelectable(lesson)
          ...
          <>
            <div className="mb-3">
              <LessonCard
                lesson={lesson}
                onEnterLesson={onEnterLesson}
                onOpenItemsList={isCurrent ? onOpenItemsList : undefined}
                isEnteringLesson={isEnteringLesson}
              />
            </div>
            {node}
...
export function InfoLearningMap({
  mapData,
}: {
  mapData: InfoMapPageData
}) {
  ...
  const completedLessonFromUrl = searchParams.get(INFO_MAP_COMPLETED_LESSON_PARAM)
  ...
  const [itemsSidebarMounted, setItemsSidebarMounted] = useState(false)
  const [itemsSidebarOpen, setItemsSidebarOpen] = useState(false)
  const [itemsSidebarLesson, setItemsSidebarLesson] = useState<MapLesson | null>(null)
  ...
  const showCalendar = INFO_CALENDAR_ENABLED

  const displayLessons = useMemo(
    () => reconcileInfoMapLessonStatuses(lessons, optimisticCompletedIds),
  ...
  useEffect(() => {
    ...
    const reconciledLessons = reconcileInfoMapLessonStatuses(
      lessons,
    ...
    params.delete(INFO_MAP_COMPLETED_LESSON_PARAM)
    const query = params.toString()
    router.replace(query ? `/invata/info?${query}` : "/invata/info", { scroll: false })
    router.refresh()
  }, [chapterKey, completedLessonFromUrl, isMapNavigating, lessons, router, searchParams])

  const handleOpenItemsSidebar = useCallback(
    (lesson: MapLesson) => {
      ...
      setItemsSidebarLesson(lesson)
      setItemsSidebarMounted(true)
      setItemsSidebarOpen(false)
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
  ...
  const closeItemsSidebar = useCallback(() => {
    setItemsSidebarOpen(false)
  }, [])

  const finalizeItemsSidebarClose = useCallback(() => {
    setItemsSidebarMounted(false)
    setItemsSidebarOpen(false)
    setItemsSidebarLesson(null)
  }, [])

  const handleSelectLesson = useCallback(
    (lessonId: string) => {
      const lesson = displayLessons.find((entry) => entry.id === lessonId)
      if (!lesson || !isInfoLessonSelectable(lesson)) return
  ...
  const chatContext = useMemo(() => buildInfoMapInsightContext(mapData), [mapData])
  const chatProblemId = useMemo(() => {
    const routeSlug = selectedRoute?.slug ?? "default"
    const chapterSlug = selectedChapter?.slug ?? "default"
    return `invata-info-map:${routeSlug}:${chapterSlug}`
  }, [selectedChapter?.slug, selectedRoute?.slug])

  const embedChatOnDesktop = chatPanelMounted && isDesktopViewport
  const embedItemsSidebarOnDesktop = itemsSidebarMounted && isDesktopViewport
  const embedRightPanelOnDesktop = embedChatOnDesktop || embedItemsSidebarOnDesktop

  const openChat = useCallback(() => {
    if (itemsSidebarMounted) {
      setItemsSidebarOpen(false)
      setItemsSidebarMounted(false)
      setItemsSidebarLesson(null)
    }
  ...
  const handleJumpToNextChapter = useCallback(() => {
    ...
    startMapTransition(() => {
      router.push(getInfoMapHref(selectedRoute.slug, nextChapter.slug))
    })
  }, [
    isCurrentChapterComplete,
    ...
    <div className="flex h-full min-h-0 flex-row">
      ...
      <div
        className={cn(
          ...
          embedRightPanelOnDesktop && "lg:mr-[25vw]",
        )}
      >
              ...
              {isMapNavigating ? (
                ...
                <>
                  <DesktopMap
                    ...
                    onOpenItemsList={handleOpenItemsSidebar}
                    isEnteringLesson={isEnteringLesson}
                  />
                  <MobileMap
                    lessons={displayLessons}
      ...
      {!chatPanelMounted && !itemsSidebarMounted && !isMapNavigating ? (
        <LearningPathItemAiChatFab
          visible
          onClick={openChat}
          showOnMobile
          className={cn(
            "bottom-6 right-5 z-[90] lg:bottom-8 lg:right-8",
      ...
      {chatPanelMounted ? (
        <InsightChatSidebar
          ...
          starterQuestionChips={INFO_INSIGHT_STARTER_CHIPS}
      ...
      {itemsSidebarMounted ? (
        <MapLessonItemsSidebar
          isOpen={itemsSidebarOpen}
          onClose={closeItemsSidebar}
