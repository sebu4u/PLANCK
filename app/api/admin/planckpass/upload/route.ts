...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const path = `cosmetics/${user.id}/${crypto.randomUUID()}.${optimized.extension}`
    const supabase = getServiceRoleSupabase()
    const { error } = await supabase.storage.from("planckpass-cosmetics").upload(path, optimized.bytes, {
      cacheControl: "31536000, immutable",
      contentType: optimized.contentType,
      upsert: false,
    })
    if (error) throw error
    const { data } = supabase.storage.from("planckpass-cosmetics").getPublicUrl(path)
    return NextResponse.json({ url: data.publicUrl, path }, { status: 201 })
  } catch (error) {
    logger.error("[admin/planckpass/upload] POST error:", error)
