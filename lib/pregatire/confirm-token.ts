...
/**
 * Mint a signed enrollment token for one-click workshop enrollment from marketing emails.
 * Format: userId:workshopId:signature
 * Signature: HMAC-SHA256(userId:workshopId) using WORKSHOP_CONFIRM_SECRET or CRON_SECRET fallback.
 * 
 * This mirrors mintConfirmToken but with clear naming for the enrollment flow.
 */
export function mintEnrollToken(userId: string, workshopId: string): string {
  const secret = process.env.WORKSHOP_CONFIRM_SECRET || process.env.CRON_SECRET || ""
  if (!secret) {
    throw new Error("WORKSHOP_CONFIRM_SECRET or CRON_SECRET required to mint enroll token")
  }
  const signature = createHmac("sha256", secret)
    .update(`${userId}:${workshopId}`)
    .digest("hex")
...
/**
 * Verify a workshop enrollment token.
 * Returns { valid: true, userId, workshopId } on success, or { valid: false, error } on failure.
