...
export const EARLYBIRD_YEARLY_SEATS_TOTAL = 20
...
/** Stable FOMO count for a calendar day — fewer yearly earlybird seats as the deadline approaches. */
export function remainingEarlybirdSeats(now = new Date()): number {
  ...
  return Math.min(EARLYBIRD_YEARLY_SEATS_TOTAL - 2, Math.max(3, daysLeft - 10))
}

export function earlybirdSeatsFomoCopy(remaining = remainingEarlybirdSeats()): string {
  return `${remaining}/${EARLYBIRD_YEARLY_SEATS_TOTAL} abonamente anuale rămase la preț de earlybird`
}
