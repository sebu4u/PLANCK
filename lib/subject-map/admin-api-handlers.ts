...
import type { SubjectMapRouteConfig } from "@/lib/subject-map/types"
...
export function createSubjectMapAdminHandlers(config: SubjectMapRouteConfig, logPrefix: string) {
  return {
    async GET(req: NextRequest) {
      try {
        ...
        if (routesErr) {
          logger.error(`[${logPrefix}] Failed to fetch routes:`, routesErr)
        ...
        if (chaptersErr) {
          logger.error(`[${logPrefix}] Failed to fetch chapters:`, chaptersErr)
        ...
        if (lessonsErr) {
          logger.error(`[${logPrefix}] Failed to fetch lessons:`, lessonsErr)
        ...
        if (assignmentsErr) {
          logger.error(`[${logPrefix}] Failed to fetch assignments:`, assignmentsErr)
        ...
        logger.error(`[${logPrefix}] GET error:`, err)
    ...
    async POST(req: NextRequest) {
      try {
        ...
        if (type === "chapter") {
          ...
          const payload = {
            route_id: routeId,
            slug: toNullableString(body.slug) ?? slugify(title),
            title,
            order_index: toInt(body.order_index, 0),
            is_active: toBoolean(body.is_active, true),
          }

          const { data, error } = await supabase.from(config.tables.chapters).insert(payload).select().single()
          if (error) {
            logger.error(`[${logPrefix}] Failed to create chapter:`, error)
        ...
        if (type === "lesson") {
          ...
          const { data, error } = await supabase.from(config.tables.lessons).insert(payload).select().single()
          if (error) {
            logger.error(`[${logPrefix}] Failed to create lesson:`, error)
            return NextResponse.json({ error: "Nu am putut crea lecția." }, { status: 500 })
          }
          return NextResponse.json({ success: true, lesson: data })
        }

        const mapLessonId = toNullableString(body.map_lesson_id ?? body[`${config.id}_lesson_id`])
        const learningPathItemId = toNullableString(body.learning_path_lesson_item_id)
        if (!mapLessonId || !learningPathItemId) {
          return NextResponse.json(
            { error: "map_lesson_id și learning_path_lesson_item_id sunt obligatorii." },
            { status: 400 },
          )
        }
        ...
        try {
          await shiftAssignmentOrderRange(supabase, config, mapLessonId, requestedOrderIndex, 1)
        } catch (shiftErr) {
          const details = shiftErr instanceof Error ? shiftErr.message : formatDbError(shiftErr)
          logger.error(`[${logPrefix}] Failed to shift assignment order:`, details)
          return NextResponse.json(
        ...
        const { data, error } = await supabase
          .from(config.tables.lessonItems)
          .insert({
            [config.tables.lessonIdColumn]: mapLessonId,
        ...
        if (error) {
          ...
          logger.error(`[${logPrefix}] Failed to create assignment:`, details)
          const message =
            error.code === "23505"
        ...
        logger.error(`[${logPrefix}] POST error:`, err)
        return NextResponse.json({ error: "Eroare internă." }, { status: 500 })
      }
    },
    ...
    async PUT(req: NextRequest) {
      try {
        ...
        if (type === "chapter") {
          ...
          if (error) {
            logger.error(`[${logPrefix}] Failed to update chapter:`, error)
        ...
        if (type === "lesson") {
          ...
          if (error) {
            logger.error(`[${logPrefix}] Failed to update lesson:`, error)
        ...
        const lessonId = (currentAssignment as Record<string, unknown>)[
          config.tables.lessonIdColumn
        ] as string
        const oldIndex = currentAssignment.order_index as number

        if (nextOrderIndex !== undefined && nextOrderIndex !== oldIndex) {
          const { data: maxOrderRows, error: maxOrderErr } = await supabase
            .from(config.tables.lessonItems)
            .select("order_index")
            .eq(config.tables.lessonIdColumn, lessonId)
            .order("order_index", { ascending: false })
            .limit(1)

          if (maxOrderErr) {
            return NextResponse.json({ error: "Nu am putut reordona asignarea." }, { status: 500 })
          }
          ...
          try {
            ...
            logger.error(`[${logPrefix}] Failed during assignment reorder:`, details)
        ...
        if (error) {
          return NextResponse.json({ error: "Nu am putut actualiza asignarea." }, { status: 500 })
        }

        return NextResponse.json({ success: true, assignment: data })
      } catch (err) {
        logger.error(`[${logPrefix}] PUT error:`, err)
        return NextResponse.json({ error: "Eroare internă." }, { status: 500 })
      }
    },
    ...
    async DELETE(req: NextRequest) {
      try {
        ...
        if (type === "chapter") {
          const { error } = await supabase.from(config.tables.chapters).delete().eq("id", id)
          if (error) {
        ...
        if (type === "lesson") {
          const { error } = await supabase.from(config.tables.lessons).delete().eq("id", id)
          if (error) {
        ...
        const { error } = await supabase.from(config.tables.lessonItems).delete().eq("id", id)
        if (error) {
        ...
        logger.error(`[${logPrefix}] DELETE error:`, err)
        return NextResponse.json({ error: "Eroare internă." }, { status: 500 })
      }
    },
  }
